...
func (e *execution) execute() {
	...
	for {
		...
		if errors.Is(retryErr, errModelNotFound) {
			e.finish(RequestStateFailed, retryErr, nil, nil)
			e.protocol.writeError(e.ctx, http.StatusBadRequest, retryErr)
			return
		}
		if errors.Is(retryErr, errUnsupportedTarget) {
			e.finish(RequestStateFailed, retryErr, nil, nil)
			e.protocol.writeError(e.ctx, http.StatusInternalServerError, retryErr)
			return
		}
		select {
		case <-ctx.Done():
...
func (e *execution) resolveTarget(ctx context.Context) (model.GroupItem, *model.Channel, int, error) {
	...
	if err != nil {
		return model.GroupItem{}, nil, retryInterval, errModelNotFound
	}
...
func (e *execution) executeAttempt(ctx context.Context, item model.GroupItem, channel *model.Channel) (bool, error) {
	...
	result := (&forwarder{protocol: e.protocol, request: &e.request, client: client}).executeUpstream(attemptCtx, item.ModelName, item.ThinkingLevel, channel)
