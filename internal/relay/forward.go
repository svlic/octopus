...
func (f *forwarder) executeUpstream(ctx context.Context, modelName, thinkingLevel string, channel *model.Channel) upstreamResult {
	passthrough := false
	switch channel.Type {
	...
	if passthrough && (thinkingLevel == "" || thinkingLevel == "default") {
		return f.executePassthrough(ctx, modelName, channel)
	}
	return f.executeConverted(ctx, modelName, thinkingLevel, channel)
}
